...
function forceClose(){
	...
	if (selectedDataList != null) {
		...
		if (confirm("有"+total+"笔安全事件信息属于IP匹配备案信息，匹配的备案信息不一定正确，是否继续强制关停操作？")) {
			var url = rootPath+"/SMMS/SmmsEventMainBiz/forceClose?RID="+rids;
			//发送一个ajax请求
...
function sand(){
	...
	if (selectedDataList != null) {
		...
		var url = rootPath+"/SMMS/SmmsEventMainBiz/Sand?RID="+rids;
		//发送一个ajax请求
		XMLHttp.urlSubmit(url,backCallSendMsg);
		function backCallSendMsg(msg){
			showMessage(msg);
...
function forceCloseAll(){
	var obj = getDataListObj('SmmsEventMain.viewAllMappingMode');
	var total=0;
	if (obj!= null) {
		...
		if (confirm("有"+total+"笔安全事件信息属于IP匹配备案信息，匹配的备案信息不一定正确，是否继续强制关停操作？")) {
			var url = rootPath+"/SMMS/SmmsEventMainBiz/forceCloseAll";
			XMLHttp.urlSubmit(url,backCallSendMsg);
			function backCallSendMsg(msg){
				showMessage(msg);
				doRefreshList();
