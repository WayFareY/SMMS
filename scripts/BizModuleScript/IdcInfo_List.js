...
function InsertSUser(){
	if(selectedRid==null || selectedRid==""){
		alert("请选择一条数据");
	}else{
		var url = rootPath+"/SMMS/IdcInfoBiz/InsertSUser?RID="+selectedRid;
		//发送一个ajax请求
		XMLHttp.urlSubmit(url,backCallSendMsg);
		function backCallSendMsg(msg){
			if(msg.indexOf('000')!=-1){
				var userId=msg.substr(4,msg.length);
				var userUrl=rootPath+"/fmp/SModuleBiz/BuildMenuXML?BUILDRANGE=5&BUILDOBJ="+userId;
				XMLHttp.urlSubmit(userUrl,null);
				showMessage("添加成功");
