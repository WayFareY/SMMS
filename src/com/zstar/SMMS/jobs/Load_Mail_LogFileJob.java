...
public class Load_Mail_LogFileJob extends BaseJob {

	public Load_Mail_LogFileJob() {
	}
	...
	public void jobExcute(String arg0) throws JobExecutionException {
		...
		while (iterator.hasNext()) {
			...
			while (iterator1.hasNext()) {
				...
				if (!fileName.startsWith("mail") || !fileName.endsWith(".mdf.ok")) {
				...
				try {
					...
					String tableRow = "USERNAME,GROUPNAME,IP,SERV_CRC,APP_CRC,LOG_TIME,CONTENT,TITLE,MAIL_FROM,MAIL_TO";
					String sqlId = "SmmsAcMail.insertSave";
					ReadAcLogDel del = new ReadAcLogDel(contex);
					dbsqlSession.delete("SmmsAcMail.deleteAll");
